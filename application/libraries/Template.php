...
class Template
{
	...
	function template_admin($content, $data = NULL)
	{
		 ...
		 *
		 */
		$data['headernya'] = $this->_ci->load->view('template_admin/v_navigasi', $data, TRUE);
		$data['content'] = $this->_ci->load->view($content, $data, TRUE);

		$this->_ci->load->view('template_admin/v_header', $data);
		$this->_ci->load->view('template_admin/v_body', $data);
		$this->_ci->load->view('template_admin/v_footer');
	}
